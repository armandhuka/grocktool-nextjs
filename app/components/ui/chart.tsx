...
const ChartStyle = ({ 
  ...
  const colorConfig = Object.entries(config).filter(([_, configItem]) => 
    configItem.theme || configItem.color
  );
      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            const indicatorColor = color || item.color || item.fill || "#ccc";

            const displayValue = formatter && item.value !== undefined 
              ? formatter(item.value, item.name || key, item, index)
...
function getPayloadConfigFromPayload(
  ...
  const payloadData = typeof payload.payload === "object" 
  ...
  if (payload[key]) {
    ...
    configLabelKey = String(payloadData[key]);
  }
