...
interface ChartTooltipContentProps extends React.HTMLAttributes<HTMLDivElement> {
  active?: boolean;
  payload?: Array<RechartsPrimitive.TooltipPayload<number, string>>;
  hideLabel?: boolean;
      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            const indicatorColor = color ?? (item.payload as any)?.fill ?? item.color ?? "#ccc";
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: Record<string, unknown>, key: string) {
  ...
  const payloadPayload = typeof payload.payload === "object" ? (payload.payload as Record<string, unknown>) : undefined;
